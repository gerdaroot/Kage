...
class Database(dict):
    ...
    async def ensure_content_channel(self):
        ...
        existing_channel_id = self.get("kage.forums", "channel_id", None)
        ...
        if existing_channel_id:
            try:
                ...
                content_channel = None
                self.set("kage.forums", "forums_cache", {"kage-userbot": {}})
        ...
        if not content_channel:
            async for dialog in self._client.iter_dialogs():
                if dialog.title and "kage-userbot" in dialog.title.lower():
                    ...
                    self.set("kage.forums", "channel_id", int(dialog.entity.id))
                    break
        ...
        if not content_channel:
            content_channel, _ = await utils.asset_channel(
                client=self._client,
                title="kage-userbot",
                description="🖤 Content related to Kage will be here",
                silent=True,
                invite_bot=True,
                avatar="https://raw.githubusercontent.com/gerdaroot/Kage/master/assets/img/avatar.png",
                forum=True,
                hide_general=True,
                _folder="kage",
            )
            self.set("kage.forums", "channel_id", int(content_channel.id))
    ...
    async def store_asset(self, message: Message) -> int:
        ...
        try:
            _assets_topic_id = self.get("kage.forums", "forums_cache", {})[
                "kage-userbot"
            ]["Assets"]
        except (TypeError, KeyError):
            raise NoAssetsChannel("Tried to save asset to non-existing asset topic.")

        if not (_content_channel_id := self.get("kage.forums", "channel_id", None)):
