...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Kage modules properly.
    """

    try:
        if filename.startswith("<") and filename.endswith(">"):
            module = filename[1:-1].split(maxsplit=1)[-1]
            if (module.startswith("kage.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.kage_caller or client_id == record.kage_caller)
        ]
    ...
    async def sender(self):
        async with self._send_lock:
            ...
            for client_id in self._mods:
                ...
                funcs = []
                for item in self.tg_buff:
                    if not isinstance(item[0], KageException):
                        continue
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_kage_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_kage_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        if record.levelno >= self.tg_level:
            if record.exc_info:
                ...
                exc = KageException.from_exc_info(
                    *record.exc_info,
