...
async def asset_forum_topic(
    ...
    async def create_topic() -> ForumTopic:
        result = await client(
            CreateForumTopicRequest(
                peer=entity,
                title=title,
                icon_emoji_id=(icon_emoji_id if client.kage_me.premium else None),
            )
        )
    ...
    forums_cache = db.pointer("kage.forums", "forums_cache", {})
...
async def wait_for_content_channel(db: "Database", delay: float = 10) -> int:
    cid = db.get("kage.forums", "channel_id", None)

    while not cid:
        logger.warning(
            "Kage content channel not found in database. Sleeping 10 seconds..."
        )
...
async def get_topic_id(db: "Database", topic_name: str) -> int | None:
    ...
    """
    try:
        forums_cache = db.get("kage.forums", "forums_cache", {})
        return forums_cache.get("kage-userbot", {}).get(topic_name)
    except Exception:
        return None
