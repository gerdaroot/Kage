...
def _is_no_git() -> bool:
    return os.environ.get("KAGE_NO_GIT") == "1"
...
def get_git_info() -> tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/gerdaroot/Kage/commit/{hash_}" if hash_ else "",
    )


def get_git_hash() -> str | Literal[False]:
    """
    Get current Kage git hash
...
def get_commit_url() -> str:
    """
    Get current Kage git commit url
    ...
    try:
        ...
        if not hash_:
            return "Unknown"
        return f'<a href="https://github.com/gerdaroot/Kage/commit/{hash_}">#{hash_[:7]}</a>'
    except Exception:
        return "Unknown"
