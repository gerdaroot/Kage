...
class CustomTelegramClient(TelegramClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._kage_entity_cache: dict[
            str | int,
            CacheRecordEntity,
        ] = {}

        self._kage_perms_cache: dict[
            str | int,
            CacheRecordPerms,
        ] = {}

        self._kage_fullchannel_cache: dict[
            str | int,
            CacheRecordFullChannel,
        ] = {}

        self._kage_fulluser_cache: dict[
            str | int,
            CacheRecordFullUser,
        ] = {}
        ...
        self.kage_me: "User"
        self.hikka_me: "User"
        self.kage_db: "Database"
        self.loader: "Modules"
        self.kage_inline: "InlineManager"
    ...
    @property
    def kage_entity_cache(self) -> dict[int, CacheRecordEntity]:
        return self._kage_entity_cache

    @property
    def kage_perms_cache(self) -> dict[int, CacheRecordPerms]:
        return self._kage_perms_cache

    @property
    def kage_fullchannel_cache(self) -> dict[int, CacheRecordFullChannel]:
        return self._kage_fullchannel_cache

    @property
    def kage_fulluser_cache(self) -> dict[int, CacheRecordFullUser]:
        return self._kage_fulluser_cache
    ...
    async def get_entity(
        ...
        _kage_client_id_logging_tag = copy.copy(self.tg_id)  # noqa: F841
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._kage_entity_cache
            and (
                not exp
                or self._kage_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._kage_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._kage_entity_cache[hashable_entity].entity)

        resolved_entity = await super().get_entity(entity)

        if resolved_entity:
            cache_record = CacheRecordEntity(hashable_entity, resolved_entity, exp)
            self._kage_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._kage_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._kage_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._kage_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._kage_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._kage_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._kage_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._kage_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: str | int):
                ...
                if getattr(user, "id", None):
                    self._kage_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._kage_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._kage_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._kage_fullchannel_cache.get(hashable_entity)
            and not self._kage_fullchannel_cache[hashable_entity].expired
            and self._kage_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._kage_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._kage_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._kage_fulluser_cache.get(hashable_entity)
            and not self._kage_fulluser_cache[hashable_entity].expired
            and self._kage_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._kage_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._kage_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def _call(
        ...
        # list and you will be banned from Kage federation.
