...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.kage_me.id
        self._cached_usernames = set()

        if self._client.kage_me.username:
            self._cached_usernames.add(self._client.kage_me.username.lower())

        if self._client.kage_me.usernames:
            self._cached_usernames.update(
                u.username.lower()
                for u in getattr(self._client.kage_me, "usernames", [])
            )

        self._cached_usernames.add(str(self._client.kage_me.id))
