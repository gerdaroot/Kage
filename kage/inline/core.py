...
class InlineManager(
    ...
    :type client: kage.tl_cache.CustomTelegramClient
    :type db: kage.database.Database
    :type allmodules: kage.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("kage.inline", "bot_token", False)
    ...
    async def register_manager(
        ...
        self._name = get_display_name(self._client.kage_me)
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"kage-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        for folder in _folders.filters:
            if getattr(folder, "title", None) == "Kage":
                if any(
                    [
    ...
    async def _ping_bot(
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start kage init")
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start kage init"
                )
            except Exception:
