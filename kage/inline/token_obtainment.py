...
class TokenObtainment(InlineUnit):
    async def _create_bot(self: "InlineManager"):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("kage.inline", "custom_bot", False):
                username = self._db.get("kage.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if self._db.get("kage.inline", "custom_bot", False) and (
                        self._db.get("kage.inline", "custom_bot", False) != btn_text
                    ):
                        continue

                    if not self._db.get("kage.inline", "custom_bot", False) and not (
                    ...
                    self._db.set("kage.inline", "bot_token", token)
                    self._token = token
