...
class InlineStuff(loader.Module):
    ...
    @loader.command()
    async def ch_kage_bot(self, message: Message):
        ...
        self._db.set("kage.inline", "custom_bot", args)
        self._db.set("kage.inline", "bot_token", None)
    ...
    async def ch_bot_token(self, message: Message):
        ...
        if not args or not re.match(r"[0-9]{8,10}:[a-zA-Z0-9_-]{34,36}", args):
            ...
            return
        self._db.set("kage.inline", "bot_token", args)
        await utils.answer(message, self.strings["bot_updated"])
        ...
        match message.text:
            case "/start":
                await message.answer_photo(
                    "https://raw.githubusercontent.com/gerdaroot/Kage/master/assets/img/start.png",
                    caption=self.strings["this_is_kage"].format(
                        (
                            "<tg-emoji emoji-id=5449692618151695997>🖤</tg-emoji>"
                            if self._client.kage_me.premium is True
                            else "🖤"
                        ),
                        utils.get_platform_emoji() if self._client.kage_me.premium is True else "Kage",
                    ),
                    reply_markup=self.inline.generate_markup(
                        markup_obj=[
                            [
                                {
                                    "text": "GitHub",
                                    "url": "https://github.com/gerdaroot/Kage",
    ...
    async def reset_prefix(self, call: InlineCall, message):
        ...
        self.db.set("kage.main", "command_prefix", ".")
