...
class Evaluator(loader.Module):
    ...
    class _SecureDB:
        ...
        def set(self, *args, **kwargs):
            if len(args) >= 2 and args[0] == "kage.security" and args[1] == "owner":
                raise ValueError(
    ...
    async def e(self, message: Message):
        ...
        try:
            ...
            item = KageException.from_exc_info(*sys.exc_info())
            print_output = output_print.getvalue()
    ...
    def censor(self, ret: str) -> str:
        ret = ret.replace(str(self._client.kage_me.phone), "&lt;phone&gt;")
        ...
        if btoken := self._db.get("kage.inline", "bot_token", False):
            ret = ret.replace(
                btoken,
                f'{btoken.split(":")[0]}:{"*" * 26}',
            )
