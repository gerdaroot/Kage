...
class Quickstart(loader.Module):
    ...
    async def client_ready(self):
        self.text = lambda: self.strings["base"].format(
            ...
            if self.client.kage_me.premium is True
            else "Kage"
        )

        try:
            content_channel = None
            existing_channel_id = self.db.get("kage.forums", "channel_id", None)
            ...
            if existing_channel_id:
                try:
                    ...
                    content_channel = None
                    self.db.set("kage.forums", "forums_cache", {"kage-userbot": {}})

            if not content_channel:
                async for dialog in self.client.iter_dialogs():
                    if dialog.title and "kage-userbot" in dialog.title.lower():
                        content_channel = dialog.entity
                        logger.debug(
                            f"Found existing channel '{dialog.title}' with ID {dialog.entity.id}"
                        )
                        self.db.set(
                            "kage.forums", "channel_id", int(dialog.entity.id)
                        )
                        break
            ...
            forum_entity = None
            existing_forum_id = self.db.get("kage.forums", "forum_id", None)
            ...
            if not forum_entity:
                try:
                    ...
                    self.db.set("kage.forums", "forum_id", int(content_channel.id))
                except Exception:
                    forum_entity = content_channel
            ...
            required_topics = [
                (
                    "Assets",
                    "🌆 Your Kage assets will be stored here",
                    5877307202888273539,
                ),
                (
                    "Backups",
                    "💾 Your Kage backups will be stored here",
                    5877307202888273539,
                ),
            ]
            ...
            [
                {
                    ...
                    "url": "https://github.com/gerdaroot/Kage/issues",
                }
            ],
        ] + utils.chunks(
            [
    ...
    async def lang(self, call: BotInlineCall):
        if not call.data.startswith("kage/lang/"):
            return
