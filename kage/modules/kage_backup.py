...
class KageBackupMod(loader.Module):
                ...
                reply_markup=self.inline.generate_markup(
                    [
                        [
                            {
                                "text": "↪️ Restore this",
                                "data": "kage/backupall/restore/confirm",
                            }
                        ]
                    ]
                ),
    ...
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("kage/backupall/restore"):
            return

        if call.data == "kage/backupall/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "kage/backupall/restore",
                },
            )
            return
    ...
    async def backupall(self, message: Message):
        ...
        archive.name = f"kage-{datetime.datetime.now():%d-%m-%Y-%H-%M}.backup"
