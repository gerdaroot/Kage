...
NO_GIT = os.environ.get("KAGE_NO_GIT") == "1"
...
class UpdaterMod(loader.Module):
    """Updates itself, tracks latest Kage releases, and notifies you, if update is required"""
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                "GIT_ORIGIN_URL",
                "https://github.com/gerdaroot/Kage",
    ...
    async def poller_announcement(self):
        async with aiohttp.ClientSession() as session:
            try:
                url = "https://api.github.com/repos/gerdaroot/Kage/contents/assets/announcement.txt"
                r = await session.get(
                    url,
    ...
    async def update_call(self, call: InlineCall):
        ...
        if NO_GIT:
            ...
            return
        if call.data not in {"kage/update", "kage/ignore_upd"}:
            return

        if call.data == "kage/ignore_upd":
            ...
            try:
                await self._client(
                    UpdateDialogFilterRequest(
                        ...
                        DialogFilter(
                            folder_id,
                            title=TextWithEntities(text="Kage", entities=[]),
                            pinned_peers=(
                                [
                ...
                logger.critical(
                    "Can't create Kage folder. Possible reasons are:\n"
    ...
    async def ubstop(self, message: Message):
        ...
        await self.inline.form(
            ...
            text=self.strings["stop_ub_confirm"].format(
                utils.get_platform_emoji()
                if self.client.kage_me.premium
                else "Kage"
            ),
            reply_markup=[
                [
