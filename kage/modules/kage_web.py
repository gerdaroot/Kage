...
@loader.tds
class KageWebMod(loader.Module):
    """Inline account management"""

    strings = {"name": "KageAccounts"}
    ...
    def _get_client(self) -> CustomTelegramClient:
        return CustomTelegramClient(
            MemorySession(),
            main.kage.api_token.ID,
            main.kage.api_token.HASH,
    ...
    def _clone_switch_db(self, old_id: int, new_id: int) -> dict:
        ...
        inline_data = data.setdefault("kage.inline", {})
    ...
    async def _save_switch_session(
        ...
        session = SQLiteSession(os.path.join(main.SESSIONS_DIR, f"kage-{new_id}"))
        session.set_dc(
    ...
    async def _switch_account(self, client: CustomTelegramClient) -> None:
        ...
        new_id = me.id
        if old_id == new_id:
            await main.kage.save_client_session(client, delay_restart=False)
            return

        suffix = int(time.time())
        base = Path(main.SESSIONS_DIR)
        old_session = base / f"kage-{old_id}.session"
        old_journal = base / f"kage-{old_id}.session-journal"
        old_config = Path(main.BASE_PATH) / f"config-{old_id}.json"
        new_session = base / f"kage-{new_id}.session"
        new_journal = base / f"kage-{new_id}.session-journal"
