...
class LoaderMod(loader.Module):
    ...
    async def approve_internal(
        ...
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/gerdaroot/Kage/master/assets/img/joined.png",
        )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?(?:kage|heroku)_min", doc):
            ver = re.search(r"# ?scope: ?(?:kage|heroku)_min ((?:\d+\.){2}\d+)", doc).group(1)
            ver_ = tuple(map(int, ver.split(".")))
            if main.__version__ < ver_:
                logger.error(
                    "Module %s requires Kage %s, current version is %s",
                    module_label,
                    ver,
        ...
        module_name = f"kage.modules.{uid}"
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "kage_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.kage_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.kage_entity_cache
                    and getattr(
    ...
    def _is_core_module(self, module) -> bool:
        ...
        if not module_name.startswith("kage.modules."):
            return False
