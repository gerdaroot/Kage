...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _kage_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?kage$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.kage_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.kage_commands)
                inline_handlers.update(module.kage_inline_handlers)
                callback_handlers.update(module.kage_callback_handlers)
                watchers.extend(module.kage_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.kage_commands))
            )

        for _command, cmd in instance.kage_commands.items():
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.kage_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.kage_watchers.values():
