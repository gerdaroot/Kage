...
class Module:
    ...
    @property
    def kage_commands(self) -> dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def kage_inline_handlers(self) -> dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def kage_callback_handlers(self) -> dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def kage_watchers(self) -> dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @kage_commands.setter
    def kage_commands(self, _):
        pass
    ...
    @kage_inline_handlers.setter
    def kage_inline_handlers(self, _):
        pass
    ...
    @kage_callback_handlers.setter
    def kage_callback_handlers(self, _):
        pass
    ...
    async def animate(
        ...
        *,
        ...
        with contextlib.suppress(AttributeError):
            _kage_client_id_logging_tag = copy.copy(self.client.tg_id)  # noqa: F841
    ...
    async def _decline(
        ...
        self._db.set(
            "kage.main",
            "declined_joins",
            list(set(self._db.get("kage.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/gerdaroot/Kage/master/assets/img/declined.png",
        )
    ...
    async def request_join(
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("kage.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
        ...
        await self.inline.bot.send_photo(
            self.tg_id,
            "https://raw.githubusercontent.com/gerdaroot/Kage/master/assets/img/join_request.png",
            caption=(
        ...
        self.kage_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "kage_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?(?:kage|heroku)_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?(?:kage|heroku)_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Kage version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"kage.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("kage.main", "stats", True)
            and url is not None
