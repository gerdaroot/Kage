...
class Kage:
    ...
    def _migrate_sessions(self):
        ...
        with os.scandir(BASE_DIR) as entries:
            legacy = [
                ...
                for entry in entries
                if entry.is_file()
                and entry.name.startswith("kage-")
                and ".session" in entry.name
            ]
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.kage_db = database.Database(client)
        await client.kage_db.init()

        try:
            db = client.kage_db
            existing = db.get("kage.inline", "custom_bot", False)
        except Exception:
            existing = False
        ...
        if (
            ...
            while bot := input(
                "You can enter a custom bot username or leave it empty and Kage will generate a random one: "
            ):
                bot = bot.strip()
                bot = bot.lstrip("@")
                if any(
                ...
                try:
                    if await self._check_bot(client, bot):
                        db.set("kage.inline", "custom_bot", bot)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            if os.environ.get("KAGE_NO_GIT") == "1":
                build = "unknown"
                upd = "Git disabled"
            else:
                import git
            ...
            pref = client.kage_db.get("kage.main", "command_prefix", None)
            ...
            try:
                ...
                await client.kage_inline.bot.send_photo(
                    log_chat_id,
                    "https://raw.githubusercontent.com/gerdaroot/Kage/master/assets/img/started.png",
                    caption=(
                        "{} <b>{} started!</b>\n\n<tg-emoji emoji-id=5231065262228250587>⚙</tg-emoji> <b>GitHub commit SHA: <a"
                        ' href="https://github.com/gerdaroot/Kage/commit/{}">{}</a></b>\n<tg-emoji emoji-id=5873225338984599714>🔎</tg-emoji>'
            ...
            logging.debug(
                ...
                client.tg_id,
                client.kage_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
